...
function CleanerRobot(
    ...
    const onReady = () =>
        console.log(`Уборка завершена. Осталось заряда батареи: ${energyConsumption(energy)}%.`);
    ...
    const energyConsumption = (energy) => {
        let energyRemain = energy - getCleaningTime() * ENERGY_CONSUMPTION;
        return energyRemain;
    }

    this.stop = () => {
        clearTimeout(timerId);
        console.log(`Уборка завершена досрочно. Осталось заряда батареи: ${energyConsumption(energy)}%.`);
    }
}
