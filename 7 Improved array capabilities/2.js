...
function collect(array) {
    if (!arguments) throw new Error('function must be called with arguments');
    if (!Array.isArray(array)) throw new Error('parameter must be an array');

    const flattened = array.flat(Infinity);
    if (flattened.some(item => typeof item !== 'number')) throw new Error('some item of array is not a number');
