...
// 4. В том случае если в массиве `countries` отсутствует необходимый `id` промис **должен** реджектится с 
//    текстом `We don't have information about country for this customer: ${customer.name}`;
...
function getCustomers(customers, countries) {
    return new Promise( (resolve, reject) => {
        ...
        for (let i = 0; i < customers.length; i++) {
            if (!countryIds.includes(customers[i].id)) {
                reject(`We don't have information about country for this customer: ${customers[i].name}`);
            }
        }
        for (let i = 0; i < customers.length; i++) {
            if (customers[i].id === countries[i].id && customers[i].verified === true) {
                customers[i].country = countries[i].country;
            }
        }
        resolve(customers);
    });
